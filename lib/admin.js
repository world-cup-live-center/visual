...
router.get("/settings", async (req, res) => {
  try {
    ...
    const subscriptionEnabled = (await getSetting("iyzico_subscription_enabled")) === "true";
    const shopierKey = (await getSetting("shopier_api_key")) || "";
    const shopierSecret = (await getSetting("shopier_secret")) || "";
    ...
    res.json({
      ...
      shopier: {
        apiKey: shopierKey,
        secretSet: Boolean(shopierSecret),
        secretHint: shopierSecret ? "••••" + shopierSecret.slice(-4) : ""
      }
    });
  } catch (error) {
...
router.post("/settings", async (req, res) => {
  ...
  try {
    ...
    await setSetting("iyzico_subscription_enabled", b.subscriptionEnabled ? "true" : "false");
    if (typeof b.shopierApiKey === "string") await setSetting("shopier_api_key", b.shopierApiKey.trim());
    if (typeof b.shopierSecret === "string" && b.shopierSecret.trim()) await setSetting("shopier_secret", b.shopierSecret.trim());
    if (b.provider === "shopier" || b.provider === "iyzico") await setSetting("payment_provider", b.provider);
    res.json({ ok: true });
  } catch (error) {
