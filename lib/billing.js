...
router.post("/checkout", jsonBody, requireDb, requireAuth, async (req, res) => {
  ...
  try {
    ...
    const form = await iyzico.initializeCheckout(req.authUser, plan, period, callbackUrl);

    // Callback cookie tasimaz; token -> kullanici eslemesini sakla.
    await db.query(
      `INSERT INTO checkout_sessions (token, user_id, plan_id, period) VALUES ($1, $2, $3, $4)
       ON CONFLICT (token) DO NOTHING`,
      [form.token, req.authUser.id, plan.id, period]
    );

    res.json({ ok: true, token: form.token, checkoutFormContent: form.checkoutFormContent });
  } catch (error) {
    console.error("[billing/checkout]", error.message);
    if (error.code === "IYZICO_UNCONFIGURED") {
      return res.status(503).json({ code: "PAYMENT_NOT_READY", error: "Ödeme sistemi henüz yapılandırılmadı. Yakında aktif olacak." });
    }
    res.status(502).json({ error: `Ödeme başlatılamadı: ${error.message}` });
  }
});
...
async function handleCheckoutCallback(req, res) {
  ...
  try {
    ...
    console.log("[billing/callback] iyzico sonucu:", JSON.stringify(result.data).slice(0, 500));
    const subRef = result.data.referenceCode || result.data.subscriptionReferenceCode || null;
    ...
    // Ayni kullanicinin onceki aktif aboneligini iyzico'da iptal et (cifte tahsilati onle).
    ...
    for (const old of oldSubs.rows) {
      if (old.iyzico_subscription_ref && old.iyzico_subscription_ref !== subRef) {
        iyzico.cancelSubscription(old.iyzico_subscription_ref)
          .catch((e) => console.warn("[billing/callback] eski abonelik iptali:", e.message));
      }
      await db.query("UPDATE subscriptions SET status = 'superseded', updated_at = now() WHERE id = $1", [old.id]);
    }

    await db.query(
      `INSERT INTO subscriptions (user_id, plan_id, period, status, iyzico_subscription_ref, current_period_start, current_period_end)
       VALUES ($1, $2, $3, 'active', $4, $5, $6)`,
      [session.user_id, session.plan_id, session.period, subRef, now, periodEnd]
    );
    await db.query("UPDATE users SET plan_id = $1 WHERE id = $2", [session.plan_id, session.user_id]);
    await db.query("UPDATE checkout_sessions SET consumed = TRUE WHERE token = $1", [token]);
